...
Now we have a multi-tab, color-coded MS Excel report with summary statistics. On the data tabs of the report, all cells from `df1` will have a white background and all cells from `df2` will have a <mark style="background-color: "#D3D3D3">grey background</mark>. The text is colored accordingly:

-  \textcolor{green}{Green text}: cell values in `df1` that are different from `df2`

-  \textcolor{red}{Red text}: cell values in `df2` that are different from `df1`
