...
-  `data_top_bottom`: the combined data displayed so that each row of `df1` is directly above its `df2` counter part.

-  `data_left_right`: the same data as shown in `data_top_bottom`, but where `df1` columns are immediately to the left of their `df2` counter part.

-  `df1`: the raw data from `df1`, for reference

-  `df2`: the raw data from `df2`, for reference

On the data tabs (`data_top_bottom` and `data_left_right`), you will see some extra columns that report useful comparison information:
